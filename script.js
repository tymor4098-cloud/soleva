  ...
  document.getElementById("products").innerHTML=list.map(p=>`
    <article class="product">
      ...
      <div class="product-info">
        <h3>${p.name}</h3><div class="meta">${p.category}</div>
...
function addToCart(id){
  ...
  if(found) found.qty++; else cart.push({...products.find(p=>p.id===id),qty:1});
  updateCart(); openCart();
}
function updateCart(){
  ...
  document.getElementById("cartItems").innerHTML=cart.length?cart.map(x=>`
    <div class="cart-row">
      ...
      <div style="flex:1"><strong>${x.name}</strong><div>${money(x.price)} × ${x.qty}</div>
      <div class="qty">Quantity: ${x.qty}</div><button class="remove" onclick="removeItem(${x.id})">Remove</button></div>
